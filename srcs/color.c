...
/*   Updated: 2025/03/11 18:28:33 by jdorazio         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	interpolate_color(int start_color, int end_color, float factor)
{
	...
	blue = ((1 - factor) * (start_color & 0xFF))
		+ (factor * (end_color & 0xFF));
	green = ((1 - factor) * ((start_color >> 8) & 0xFF))
		+ (factor * ((end_color >> 8) & 0xFF));
	red = ((1 - factor) * ((start_color >> 16) & 0xFF))
		+(factor * ((end_color >> 16) & 0xFF));
